...
export async function fetchPackage(spec: string, force = false): Promise<PackageData> {
  return tracer.startActiveSpan('taze.fetch.package', async (span) => {
    try {
      ...
      if (result.tags != null && result.tags.latest != null) {
        span.setAttribute('taze.package.latest_version', result.tags.latest)
      }
      return result
    } catch (error) {
...
export async function fetchJsrPackageMeta(name: string): Promise<PackageData> {
  return tracer.startActiveSpan('taze.fetch.jsr_package_meta', async (span) => {
    try {
      ...
      if (meta.latest != null) {
        span.setAttribute('taze.package.latest_version', meta.latest)
      }
      return {
