...
export async function writeJSON(filepath: string, data: Record<string, unknown>) {
  return tracer.startActiveSpan('taze.packages.write_json', async (span) => {
    try {
...
export async function loadPackage(
  ...
  return tracer.startActiveSpan('taze.packages.load_package', async (span) => {
    try {
      span.setAttribute('taze.package_json.filepath', relative)
...
export async function loadPackages(options: CommonOptions): Promise<PackageMeta[]> {
  return tracer.startActiveSpan('taze.packages.load_packages', async (span) => {
    try {
