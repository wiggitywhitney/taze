...
export async function readYAML(filepath: string): Promise<DocumentType> {
  return tracer.startActiveSpan('taze.package.read_yaml', async (span) => {
    try {
...
export async function writeYAML(filepath: string, data: DocumentType | Record<string, unknown>) {
  return tracer.startActiveSpan('taze.write.yaml', async (span) => {
    try {
...
export async function loadPackageYAML(
  ...
  return tracer.startActiveSpan('taze.package.load_package_yaml', async (span) => {
    try {
      ...
      span.setAttribute('taze.package.deps_count', deps.length)
...
export async function writePackageYAML(
  ...
  return tracer.startActiveSpan('taze.write.package_yaml', async (span) => {
    try {
