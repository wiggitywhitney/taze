...
export async function loadYarnWorkspace(
  ...
  return tracer.startActiveSpan('taze.package.load_yarn_workspace', async (span) => {
    try {
      ...
      span.setAttribute('taze.catalog.count', catalogs.length)
      return catalogs
    } catch (error) {
...
export async function writeYarnWorkspace(
  ...
  return tracer.startActiveSpan('taze.write.yarn_workspace', async (span) => {
    try {
