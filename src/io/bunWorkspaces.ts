...
export async function loadBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.bun_workspace.load', async (span) => {
    try {
      ...
      span.setAttribute('taze.bun_workspace.catalogs_count', catalogs.length)
      return catalogs
    } catch (error) {
...
export async function writeBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.bun_workspace.write', async (span) => {
    try {
