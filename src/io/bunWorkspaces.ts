...
export async function loadBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.io.load_bun_workspace', async (span) => {
    try {
      ...
      span.setAttribute('taze.io.catalogs_found', catalogs.length)
      return catalogs
    } catch (error) {
...
export async function writeBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.io.write_bun_workspace', async (span) => {
    try {
...
async function writeBunJSON(filepath: string, data: Record<string, unknown>) {
  return tracer.startActiveSpan('taze.io.write_bun_json', async (span) => {
    try {
