...
export async function loadBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.package.load_bun_workspace', async (span) => {
    try {
      ...
      span.setAttribute('taze.catalog.count', String(catalogs.length))
      return catalogs
    } catch (error) {
...
export async function writeBunWorkspace(
  ...
  return tracer.startActiveSpan('taze.write.bun_workspace', async (span) => {
    try {
      if (pkg != null) {
...
async function writeBunJSON(filepath: string, data: Record<string, unknown>) {
  return tracer.startActiveSpan('taze.write.bun_json', async (span) => {
    try {
