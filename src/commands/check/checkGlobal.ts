...
export async function checkGlobal(options: CheckOptions) {
  return tracer.startActiveSpan('taze.check.global', async (span) => {
    try {
      ...
      span.setAttribute('taze.check.packages_total', pkgs.reduce((sum, p) => sum + p.deps.length, 0))
      span.setAttribute('taze.check.packages_outdated', pkgs.reduce((sum, p) => sum + p.resolved.filter(j => j.update).length, 0))
...
async function loadGlobalPnpmPackage(options: CheckOptions): Promise<GlobalPackageMeta[]> {
  return tracer.startActiveSpan('taze.check.load_global_pnpm', async (span) => {
    try {
      let pnpmStdout
      ...
      span.setAttribute('taze.config.sources_found', pkgMetas.length)
      return pkgMetas
    }
    catch (error) {
      span.recordException(error instanceof Error ? error : new Error(String(error)))
...
async function loadGlobalNpmPackage(options: CheckOptions): Promise<GlobalPackageMeta> {
  return tracer.startActiveSpan('taze.check.load_global_npm', async (span) => {
    try {
...
async function installPkg(pkg: GlobalPackageMeta) {
  return tracer.startActiveSpan('taze.check.install_pkg', async (span) => {
    try {
