...
export async function check(options: CheckOptions) {
  return tracer.startActiveSpan('taze.check.run', async (span) => {
    try {
      ...
      const packagesTotalDeps = resolvePkgs.reduce((acc, pkg) => acc + pkg.deps.length, 0)
      const packagesOutdatedDeps = resolvePkgs.reduce((acc, pkg) => acc + pkg.resolved.filter(dep => dep.update).length, 0)
      span.setAttribute('taze.check.packages_total', packagesTotalDeps)
      span.setAttribute('taze.check.packages_outdated', packagesOutdatedDeps)
