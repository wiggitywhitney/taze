...
export async function check(options: CheckOptions) {
  return tracer.startActiveSpan('taze.check.command', async (span) => {
    try {
      ...
      const packagesTotal = resolvePkgs.reduce((acc, pkg) => acc + pkg.resolved.length, 0)
      const packagesOutdated = resolvePkgs.reduce((acc, pkg) => acc + pkg.resolved.filter(d => d.update).length, 0)
