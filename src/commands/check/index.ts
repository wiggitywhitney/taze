...
export async function check(options: CheckOptions) {
  return tracer.startActiveSpan('taze.command.check', async (span) => {
    try {
      ...
      span.setAttribute('taze.check.packages_total', resolvePkgs.reduce((acc, pkg) => acc + pkg.resolved.length, 0))
      span.setAttribute('taze.check.packages_outdated', resolvePkgs.reduce((acc, pkg) => acc + pkg.resolved.filter(d => d.update).length, 0))
